...
with open(INPUT_PATH) as input:
    df = pandas.DataFrame([
        dict([[pair.group("key"), pair.group("value")]
              for pair in re.finditer(r"\b(?P<key>\w{3}):(?P<value>[^\s]+)\b", line)])
    ...
    strict_valid_df = valid_df[
        ...
        (  # sizes
            (  # cm
                ...
                (valid_df["hgt"].str.extract(r"(?P<size>[0-9]+)cm")
                 ["size"].astype("float").astype("Int64") <= 193)
            ) |
            (  # inches
                (valid_df["hgt"].str.extract(r"(?P<size>[0-9]+)in")["size"].astype("float").astype("Int64") >= 59) &
                (valid_df["hgt"].str.extract(r"(?P<size>[0-9]+)in")
                 ["size"].astype("float").astype("Int64") <= 76)
            )
        ) &
