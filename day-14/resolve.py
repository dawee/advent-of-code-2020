...
def sum_mem_values(lines, version=1):
    ...
    for line in lines:
        ...
        if mask_match:
            ...
            and_mask = int(mask_expression.replace('0', '1').replace(
                'X', '0') if version == 2 else mask_expression.replace('X', '1'), 2)
            floating_increment_parts = [
                2 ** (len(line) - index - 2) for index, char in enumerate(line) if char == 'X']
            floating_increments = [0] + sorted([sum(combination) for length in range(1, len(
                floating_increment_parts) + 1) for combination in combinations(floating_increment_parts, length)])
        else:
