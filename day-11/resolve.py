...
def build_first_matrix(data):
    lines = data.split("\n")
    matrix = dict([
        [row, dict([[col, {"taken": False, "adjacent": [], "row": row, "col": col}]
                    for col, char in enumerate(line) if char == 'L'])]
...
def seats(matrix): return (seat for row, seats in matrix.items()
                           for col, seat in seats.items())
...
def compute_first_solution(data):
    ...
    while True:
        def adjancent_taken_seat(seat): return [
            other for other in seat["adjacent"] if other["taken"]]
        seats_to_take = [seat for seat in seats(
            matrix) if not seat["taken"] and len(adjancent_taken_seat(seat)) == 0]
        seats_to_leave = [seat for seat in seats(
            matrix) if seat["taken"] and len(adjancent_taken_seat(seat)) >= 4]
...
def compute_second_solution(data):
    ...
    while True:
        ...
        seats_to_take = [seat for seat in seats(
            matrix) if not seat["taken"] and len(adjancent_taken_seat(seat)) == 0]
        seats_to_leave = [seat for seat in seats(
            matrix) if seat["taken"] and len(adjancent_taken_seat(seat)) >= 5]
